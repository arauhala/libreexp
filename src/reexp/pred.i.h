...
#ifndef REEXP_PRED_I_H_
#define REEXP_PRED_I_H_
...
namespace reexp {
	...
	template <typename P>
	int rel_inputvars<P>::maskState(int dmask, int smask) const {
	...
	template <typename P>
	bool rel_inputvars<P>::includesRelState(int istate, int rstate) const {
		int i = predicted_;
		...
		return true;
			 ...
			 */
	template <typename P>
	std::vector<double> pred<P>::reexpress(const reexp::data<P>& d) {
	...
	void pred<P>::getInputStateLogDepA(const rel_stats<P>& rs,
...
/*					double predVarTrue = 2*p;
	...
	template <typename P>
	template <typename Out>
	void pred<P>::getInputStateLogDepB(const rel_stats<P>& rs,
							  const rel_inputvars<P>& riv,
							  inputstate_logdep& inputStateLogDep,
							  Out& explain) const {
	...
	template <typename P>
	template <typename Out>
	void pred<P>::getInputStateLogDepB(int n,
...
//				const reexp::rel<P>& r = rs.data().rel();
	...
	template <typename P>
	template <typename Out>
	inline void pred<P>::getInputStateLogDep(const reexp::rel_stats<P>& rs,
									const rel_inputvars<P>& riv,
									inputstate_logdep& inputStateLogDep,
									Out& explain) const {
		if (!useLogDepB_) {
	...
	template <typename P>
	template <typename Out>
	std::vector<double> pred<P>::bitP(const data<P>& d,
							 int varid,
							 Out& explain) const {
	...
	template <typename P>
	template <typename Out>
	std::vector<double> pred<P>::rowP(const data<P>& d, int varid, Out explain) const {
		const data_var<P>& dv = d.var(varid);
		const var<P>& v = dv.var_;
		const cvec<P>& dim = dv.dim();
	...
	template <typename P>
	std::vector<double> pred<P>::p(const data<P>& d, int varid) const {
		const data_var<P>& dv = d.var(varid);
		const var<P>& v = dv.var_;
		if (v.ctx().v_[0] < 0) {
			return rowP(d, varid);
	...
	template <typename P>
	void pred<P>::info(const std::vector<double>& ps,
			  	  	   const data<P>& d,
			  	  	   int varid,
			  	  	   double& totalinfo,
	...
	template <typename P>
	group_scaler<P>::group_scaler(const pred<P>& pr, int var, int target_group_sz)
	: scales_(), avers_() {
		// try to fit at least 128 items in each group
		int groups = 0;
		if (target_group_sz > 0) {
			groups = pr.stats().data().var(var).states().size() / target_group_sz;
		}
		...
		if (groups > 3) {
			...
			for (size_t i = 0; i < ps.size(); ++i) {
				pi.push_back(std::pair<double, int>{ps[i], i});
			}
			...
			for (int i = 0; i < groups; ++i) {
				...
				double p_aver = (sum_p + 2*e_p) / double(group_sz + 2);
				double group_aver = (f + 2*e_p) / double(group_sz + 2);
	...
	template <typename P>
	void group_scaler<P>::scale(std::vector<double>& ps) {
		if (avers_.size()) {
			for (double& p : ps) {
				if (p >= avers_[0]) {
					p *= scales_[0];
